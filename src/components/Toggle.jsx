...
const Toggle = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <>
            {shouldRender && (
                <div
                    className={`fixed top-0 left-0 h-full w-full bg-white/80 backdrop-blur-xl transition-opacity duration-300 overflow-y-auto ${isVisible ? "opacity-100" : "opacity-0"
                        }`}
                >
                    {/* Botón de cerrar */}
